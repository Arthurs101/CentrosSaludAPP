...
const getPacienteFullName = "SELECT * FROM PACIENTE WHERE nombres ILIKE $1 AND apellidos IILIKE $2";
const getPacienteName = "SELECT * FROM PACIENTE WHERE nombres ILIKE $1";
const getPacienteLastName = "SELECT * FROM PACIENTE WHERE apellidos ILIKE $1";
...
FROM public."cirugía" a INNER JOIN public."médico" b ON no_colegiado_medico_encargado = "número_de_colegiado" 
...
WHERE id_paciente = $1`
const getEvolucionSurgeries = `SELECT id as id_entrada , "observación" , fecha FROM public."evolucion_cirugía" WHERE "cirujía_id" = $1`
